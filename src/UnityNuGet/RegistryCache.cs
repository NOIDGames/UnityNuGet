...
namespace UnityNuGet
{
    ...
    public class RegistryCache
    {
        ...
        private readonly PackageBuilder _converter;
            ...
            string packageNameNuGetPostFix, RegistryTargetFramework[] targetFrameworks, ILogger logger)
        {
            ...
            _converter = new PackageBuilder(_targetFrameworks, _logger) {
                OutputDir = _rootPersistentFolder,
            };
        }
        ...
        )
        {
            ...
            if (!IsUnityPackageValid(identity, npmPackageVersion) || !IsUnityPackageSha1Valid(identity, npmPackageVersion))
            {
                ...
                if (await _converter.BuildUPM(identity, downloadResult.PackageReader, unityPackage, packageEntry.Analyzer,
                        packageEntry.DefineConstraints, await PackageBuilder.GetLicenseText(packageMeta))
                    is string unityPackageFilePath)
                {
                    LogInformation($"Converted NuGet package {identity} to Unity `{unityPackageFilePath}`");
                    ...
                    var repoMeta = downloadResult.PackageReader.NuspecReader.GetRepositoryMetadata();
